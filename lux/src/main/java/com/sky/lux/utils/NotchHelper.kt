...
object NotchHelper {
    ...
    private const val TAG = "LuxNotchHelper"
    ...
    fun getNotchHeightInVivo(context: Context?): Int {
        return ScreenUtil.dp2px(context, 27)
    }
    ...
    fun getNotchHeightInXiaomi(context: Context?): Int {
        ...
        } else StatusBarHelper.getStatusBarHeight(context)
    }
    ...
    private fun getRectInfoRotation0(context: Context?): Rect? {
        val rect = Rect()
        if (DeviceUtil.isVivo()) { // TODO vivo 显示与亮度-第三方应用显示比例
            rect.top = getNotchHeightInVivo(context)
            rect.bottom = 0
        } else if (DeviceUtil.isOppo()) { // TODO OPPO 设置-显示-应用全屏显示-凹形区域显示控制
