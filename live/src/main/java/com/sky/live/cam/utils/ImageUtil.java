...
import com.sky.live.cam.opengl.GlUtil;
import com.sky.live.cam.opengl.ProgramManager;
...
public class ImageUtil {
    ...
    public  int transferTextureToTexture(int inputTexture, Constants.TextureFormat inputTextureFormat,
                                 Constants.TextureFormat outputTextureFormat,
                                 int width, int height, Transition transition) {
            if (outputTextureFormat != Constants.TextureFormat.Texure2D){
    ...
    public ByteBuffer transferTextureToBuffer(int texture, Constants.TextureFormat inputTextureFormat,
                                              Constants.PixlFormat outputFormat, int width, int height, float ratio){
        if (outputFormat != Constants.PixlFormat.RGBA8888){
            return  null;
        }
        if (null == mProgramManager) {
            mProgramManager = new ProgramManager();
    ...
    public Bitmap transferTextureToBitmap(int texture, Constants.TextureFormat inputTextureFormat,
                                          int width, int height) {
        ByteBuffer buffer = transferTextureToBuffer(texture, inputTextureFormat, Constants.PixlFormat.RGBA8888,
                width, height, 1);
        if (buffer == null) {
            return null;
        }
        return transferBufferToBitmap(buffer, Constants.PixlFormat.RGBA8888, width, height);
    }
    ...
    public int transferBufferToTexture(ByteBuffer buffer, Constants.PixlFormat inputFormat,
                                       Constants.TextureFormat outputFormat, int width, int height){

        if (inputFormat != Constants.PixlFormat.RGBA8888){
            return GlUtil.NO_TEXTURE;
        }

        if (outputFormat != Constants.TextureFormat.Texure2D){
            return GlUtil.NO_TEXTURE;
        }
    ...
    public ByteBuffer transferBufferToBuffer(ByteBuffer buffer, Constants.PixlFormat inputFormat,
                                             Constants.PixlFormat outputFormat, int width, int height){
        return null;
    ...
    public Bitmap transferBufferToBitmap(ByteBuffer buffer, Constants.PixlFormat format,
                                  int width, int height){
    ...
    public void drawFrameOnScreen(int textureId, Constants.TextureFormat srcTetxureFormat, int surfaceWidth, int surfaceHeight, float[]mMVPMatrix) {
        if (null == mProgramManager) {
            mProgramManager = new ProgramManager();
        }
